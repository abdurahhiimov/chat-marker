#!/usr/bin/env python3
"""Собирает библиотеку выдержек в папке Google Drive.

Делает две вещи за один запуск:
  1. забирает свежие выгрузки из ~/Downloads в Inbox (с бэкапом предыдущей);
  2. пересобирает структуру — темы, страницы, индекс, таблицу, дашборд.

Модель данных: у выдержки есть тема (`tag`) и цвет (`color`, обычный hex, взятый
у темы). Старые выгрузки, где темы жили хештегами в заметке, тоже читаются.

Из терминала:
    python3 library.py              # обычный прогон
    python3 library.py --no-ingest  # только пересборка
    python3 library.py --force      # пересобрать, даже если ничего не менялось
"""
...
FOLDERS = {
    ...
    "topics":   "01 Темы",
    "chats":    "02 Страницы",
    "untagged": "03 Без темы",
    "drafts":   "04 Черновики",
    "docs":     "06 Документы",
    "archive":  "09 Архив",
}

# папки прошлых версий — чистим, чтобы не мозолили глаза
LEGACY_FOLDERS = ["03 По смыслу", "02 Чаты", "05 Связи"]
...
def find_drive_root() -> Path:
    ...
    if not candidates:
        raise SystemExit(
            "Не нашёл Google Drive на этом маке.\n"
            "Поставь Google Drive для компьютера (google.com/drive/download), войди в аккаунт,\n"
            "дождись появления папки и запусти установку ещё раз.\n"
            "Либо задай путь вручную: CHATMARKER_ROOT=/путь/к/папке python3 library.py"
        )
    ...
    for old in LEGACY_FOLDERS:
        ...
        for f in list(d.glob("*.md")):
            try:
                ...
                pass
        try:
            d.rmdir()          # уйдёт, только если пуста — своё не тронем
        except OSError:
            pass
    return paths
...
TOMB_KEEP = 5000     # сколько надгробий помнит канонический файл


def _dicts(value) -> list[dict]:
    """Список словарей из чего угодно — битые формы отбрасываются молча."""
...
def _payload(data) -> dict:
    """Нормализованная выгрузка: любая форма приводится к одному виду."""
...
def merge_export(canon: dict, fresh: dict) -> dict:
    """Слияние выгрузки в канонический файл.

    Правила простые: запись из свежей выгрузки побеждает свою старую версию;
    записи, которых в выгрузке нет, остаются (это вытеснение буфера или другое
    устройство, не удаление); удаление — только по явному надгробию.
    """
    ...
    for t in fresh["tags"]:
        ...
        if name:
            by_name[name] = t
            tag_tombs.discard(name)     # тема пересоздана — надгробие снято
...
def ingest(root: Path, paths: dict[str, Path]) -> int:
    """Забирает выгрузки из Загрузок и СЛИВАЕТ их в канонический файл.

    Раньше выгрузка перезаписывала Inbox целиком — два устройства стирали
    друг друга, а вытесненное буфером исчезало из библиотеки. Теперь Inbox
    накопительный: перезаписи нет, есть слияние по id.
    """
    ...
    for f in files:
        ...
        try:
            ...
            pass         # второй процесс успел раньше — не страшно, слияние идемпотентно
        moved += 1
...
def load_all(paths: dict[str, Path]) -> tuple[list[dict[str, Any]], dict[str, str], set[str]]:
    """Возвращает выдержки, палитру тем (имя -> цвет) и темы с живым документом."""
    items: list[dict[str, Any]] = []
    palette: dict[str, str] = {}
    docs: set[str] = set()
    decided: set[str] = set()     # флаг «вести документ» берём из самой свежей выгрузки
    seen: set[str] = set()
    ...
    for f in files:
        try:
            ...
            log(f"  ! не смог прочитать {f.name}, пропускаю")
            continue
...
def fmt_of(h: dict[str, Any]) -> list[str]:
    """Форматы выдержки. Новый формат — список fmt, старый — одиночный style."""
    f = h.get("fmt")
...
def styled(h: dict[str, Any]) -> str:
    """Как выдержка выглядит в тексте: форматы накладываются разметкой."""
...
def slug(s: str, limit: int = 60) -> str:
    s = re.sub(r"[\\/:*?\"<>|\n\r\t]", " ", s or "").strip()
    s = re.sub(r"\s+", " ", s)
    return s[:limit].rstrip() or "без названия"
...
def build_topics(items, paths) -> list[tuple[str, int]]:
    ...
    for tag, hs in by_tag.items():
        body = [HEADER, f"# #{tag}",
                f"_{len(hs)} выдержек · обновлено {datetime.now():%Y-%m-%d %H:%M}_", ""]
        by_src: dict[str, list[dict]] = defaultdict(list)
        for h in hs:
            by_src[h.get("title") or "без названия"].append(h)
...
def build_sources(items, paths) -> list[tuple[str, int, str]]:
    by_src: dict[str, list[dict]] = defaultdict(list)
    for h in items:
        by_src[h.get("title") or h.get("conv") or "без названия"].append(h)
    ...
    for src, hs in by_src.items():
        ...
        body = [HEADER, f"# {src}", f"_{len(hs)} выдержек · последняя {last[:10]}_", ""]
        if url:
            body.append(f"[открыть источник]({url})\n")
...
def build_untagged(items, paths) -> int:
    ...
    body = [HEADER, "# Без темы",
            f"_{len(hs)} выдержек · обновлено {datetime.now():%Y-%m-%d %H:%M}_",
            "", "Разложи их по темам в боковой панели браузера — и они уедут отсюда.", ""]
    body += [entry_md(h) for h in hs]
    write(paths["untagged"] / "Разобрать.md", "\n".join(body))
    return len(hs)
...
def build_index(items, paths, root, topics, sources, untagged, palette) -> None:
    ...
    lines = [
        HEADER, "# Индекс выдержек",
        f"_обновлено {datetime.now():%Y-%m-%d %H:%M}_", "",
        f"Всего **{total}**, с заметкой **{with_note}**, без темы **{untagged}**, тем **{len(topics)}**.",
        "", "## Темы", "",
    ]
    if topics:
        lines += ["| тема | сколько | файл |", "|---|---|---|"]
        for t, n in topics[:60]:
            lines.append(f"| #{t} | {n} | [{t}](<{FOLDERS['topics']}/{slug(t)}.md>) |")
    else:
        lines.append("_Тем пока нет. Выдели текст в браузере и создай первую прямо на месте._")

    if untagged:
        lines += ["", f"[Разобрать без темы — {untagged} шт.](<{FOLDERS['untagged']}/Разобрать.md>)"]

    lines += ["", "## Откуда", "", "| источник | сколько | последняя |", "|---|---|---|"]
...
def xls_safe(v):
    """openpyxl падает на управляющих символах — а они реально встречаются
    в тексте, скопированном с веба."""
...
def build_xlsx(items, root, palette) -> bool:
    ...
    except Exception as e:          # файл открыт в Excel, залочен Drive и т.п.
        log(f"  ! таблица не собралась: {e}")
        return False
...
def _build_xlsx(items, root, palette) -> bool:
    ...
    ws = wb.active
    ws.title = "Выдержки"
    headers = ["Дата", "Тема", "Выдержка", "Заметка", "Источник", "Сайт", "Ссылка"]
    ws.append(headers)
    ...
    wb.save(root / "Выдержки.xlsx")
    return True


# ---------------------------------------------------------------- живые документы

SESSION_GAP = 30 * 60          # перерыв больше получаса — новый заход, как в дашборде


def _sessions(hs: list[dict[str, Any]]) -> list[list[dict[str, Any]]]:
    """Выдержки по порядку, разбитые на заходы."""
    def ts(h):
        try:
...
def _session_title(h: dict[str, Any]) -> str:
    ...
    try:
        ...
        return raw[:16].replace("T", " ") or "без даты"


def _left_bar(par, hex_color: str) -> None:
    """Цветная полоска слева — тот же приём, что в дашборде."""
...
def topic_docx(tag: str, hs: list[dict[str, Any]], color: str, path: Path) -> None:
    """Живой документ по теме: та же последовательность, что в дашборде."""
    ...
    word = _plural(len(hs), "выдержка", "выдержки", "выдержек")
    srun = sub.add_run(f"{len(hs)} {word} · обновлено {datetime.now():%Y-%m-%d %H:%M}")
    srun.font.size = Pt(9)
    srun.font.color.rgb = RGBColor(0x8B, 0x8D, 0x96)
    note = sub.add_run("\nФайл пересобирается сам при каждой выгрузке — правки здесь затрутся.")
    note.font.size = Pt(9)
    note.italic = True
    ...
    for chunk in _sessions(hs):
        ...
        for h in chunk:
            ...
            fu = fragment_url(h)
            if fu:
                _hyperlink(meta, fu, "к месту в источнике")

    tail = doc.add_paragraph()
    tail.alignment = WD_ALIGN_PARAGRAPH.CENTER
    tr = tail.add_run("собрано Chat Marker")
    tr.font.size = Pt(8)
...
def build_docs(items, paths, docs: set[str], palette) -> list[str]:
    """Пишем документ только по отмеченным темам. Чужие файлы не трогаем."""
    folder = paths["docs"]
    ...
    for tag in sorted(docs):
        ...
        if have_docx:
            ...
            try:
                ...
                made.append(tag)
            except Exception as e:                      # документ не должен ронять сборку
                log(f"  ! документ по теме «{tag}» не собрался: {e}")
        else:
            wanted.add(f"{name}.md")
            body = [HEADER, f"# #{tag}",
                    f"_{len(hs)} выдержек · обновлено {datetime.now():%Y-%m-%d %H:%M}_",
                    "", "_Для .docx не хватает библиотеки python-docx:_",
    ...
    # тема, с которой сняли галочку, свой файл забирает с собой
    ...
    readme = folder / "Что здесь.md"
    write(readme, f"""{HEADER}# Документы по темам

Здесь лежат живые документы: по одному на тему, которую ты отметил в браузере
кнопкой «вести документ» (панель выдержек → список тем → значок документа).

Файл пересобирается целиком при каждой выгрузке, поэтому правки внутри затрутся —
пиши свои мысли в `{FOLDERS['drafts']}`.

Сейчас ведутся: {', '.join('#' + t for t in sorted(made)) if made else '— ни одной темы не отмечено'}.
""")
    return made
...
def build_dashboard(items, root, palette, docs=frozenset()) -> None:
    ...
    page = DASHBOARD_HTML
    def js(value) -> str:
        # </script> внутри данных закрыл бы блок скрипта — единственная настоящая
        # дыра дашборда. U+2028/2029 в старых движках рвут строку — заодно.
    ...
    write(root / "Дашборд.html", page)


DASHBOARD_HTML = """<!doctype html>
<html lang="ru"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Выдержки</title>
<style>
...
<header>
  <div class="top">
    ...
    <button class="btn" id="copyAll">__ICON_COPY__ Копировать</button>
    <button class="btn" id="dl">__ICON_DL__ Выгрузить как есть</button>
    <button class="btn primary" id="ask">__ICON_AI__ Попросить Claude разобрать</button>
  </div>
  <div class="searchbox">__ICON_SEARCH__<input type="search" id="q" placeholder="Поиск по тексту, заметкам, источникам"></div>
...
const DOCS = new Set(__DOCS__);   // темы, по которым ведётся живой документ
const GAP = 30 * 60 * 1000;      // перерыв больше получаса — новый заход

/* Фильтр: клик по имени переключает на тему, галочка закрепляет.
   Закреплённые складываются, текущая добавляется к ним. */
const state = { q:"", current:null, pinned:new Set(), menu:false };

const counts = {};
DATA.forEach(h => (h.g.length ? h.g : ["без темы"]).forEach(t => counts[t] = (counts[t]||0)+1));
const lastSeen = {};
DATA.forEach(h => (h.g.length ? h.g : ["без темы"]).forEach(t => {
...
function match(h) {
  const on = active();
  if (on.size) {
    const own = h.g.length ? h.g : ["без темы"];
...
function chipHtml(t) {
  ...
  return `<span class="chip ${on?"on":""}" data-t="${esc(t)}">
      ...
      ${DOCS.has(t) ? `<span class="doc" title="По теме ведётся документ">__ICON_DOC__</span>` : ""}
      <span class="box ${pin?"pin":""}" data-pin="${esc(t)}" title="Закрепить">✓</span></span>`;
}

function chips() {
  const shown = byRecent.slice(0, 5);
  document.getElementById("tags").innerHTML =
    `<span class="chip plain ${active().size?"":"on"}" data-all="1">Все</span>`
...
function menuHtml(filter) {
  ...
  return `<div class="pop" id="menu">
      <input type="text" id="menuq" placeholder="Найти тему…" value="${esc(filter||"")}">
      <div class="rows">${list.length ? list.map(t => `
        <button class="row" data-t="${esc(t)}">
          ...
          <span class="box ${state.pinned.has(t)?"pin":""}" data-pin="${esc(t)}" title="Закрепить">✓</span>
        </button>`).join("") : '<div class="empty" style="padding:24px 0">ничего не нашлось</div>'}</div>
    </div>`;
}
...
const PAGE_SIZE = 400;                    // карточек за раз: длинные ленты рендерим порциями
  ...
  return `<div class="item">
      ...
      <div class="body" style="border-left-color:${h.c}">
        ...
        <div class="meta">
          ...
          <span>${esc(h.s)}</span>
          <span>${h.d}</span>
          ${fragUrl(h) ? `<a href="${esc(fragUrl(h))}" target="_blank" rel="noopener">__ICON_EXT__ к месту</a>` : ""}
          <button data-copy="${i}">__ICON_COPY__ копировать</button>
        </div>
      </div></div>`;
}
...
function render(keepShown) {
  ...
  document.getElementById("sub").textContent =
    `${r.length} из ${DATA.length}` + (on.length ? ` · ${on.join(", ")}` : ` · ${byRecent.length} тем`);

  if (!r.length) { document.getElementById("view").innerHTML = '<div class="empty">Ничего не нашлось</div>'; return; }
  ...
  if (r.length > slice.length) {
    html += `<button class="btn more" id="more">Показать ещё ${Math.min(PAGE_SIZE, r.length - slice.length)} из ${r.length - slice.length}</button>`;
  }
...
document.addEventListener("click", e => {
  ...
  if (e.target.closest("#copyAll")) {
    navigator.clipboard.writeText(asText(rows())); return toast("Тема скопирована");
  }
...
function topicName() {
  const on = [...active()];
  return on.length ? on.join(", ") : (state.q || "все темы");
}

function fileName(ext) {
  const d = new Date().toISOString().slice(0,10);
  const t = topicName().replace(/[\\\\/:*?"<>|]/g, " ").trim().slice(0, 60) || "выдержки";
  return `${t} — ${d}.${ext}`;
}

/* Выгрузить как есть: markdown-конспект из того, что сейчас в фильтре.
   Без обработки — просто последовательность, как отмечал. */
function asDoc(list) {
  const head = `# ${topicName()}\\n\\n_${list.length} выдержек · собрано ${new Date().toLocaleDateString("ru-RU")}_\\n\\n`;
...
function downloadPlain() {
  const r = rows();
  if (!r.length) return toast("Нечего выгружать");
...
/* Попросить Claude разобрать: открываем Claude с задачей собрать документ.
   Данные он возьмёт сам через MCP highlights — здесь только первые выдержки,
   чтобы разговор начался с контекстом, даже если MCP не подключён. */
function askClaude() {
  const r = rows();
  if (!r.length) return toast("Нечего разбирать");
  const topic = topicName();
  let q = `Собери конспект по теме «${topic}» из моих выдержек.\\n\\n`
        + `1. Возьми полную последовательность через MCP highlights: get_topic_map(topic: "${topic}"), при нехватке — search_highlights.\\n`
        + `2. Разбери: к чему я шёл, что повторяется, где противоречия, чего не хватает. Цитаты оставь дословно, ссылки «к месту» сохрани.\\n`
        + `3. Готовый текст положи в Google Doc «${topic} — конспект» в папку AI Highlights через коннектор Google Drive, потом дай ссылку.\\n\\n`
        + `Вот эти же выдержки в порядке, в котором я их отмечал (${r.length} шт.):\\n`
        + r.slice(0,60).map((h,i) => `${i+1}. ${h.t}` + (h.n ? ` — заметка: ${h.n}` : "")).join("\\n");
...
# Иконки нарисованы вручную в манере Tabler: сетка 24, обводка 2, скруглённые концы.
ICONS = {
...
def _icons(page: str) -> str:
    """Иконки вставляем с одинарными кавычками: они попадают и внутрь
    JS-строк с двойными кавычками, и ломать там ничего не должны."""
...
def build_readme(root: Path) -> None:
    p = root / "Как здесь всё устроено.md"
    write(p, f"""{HEADER}# Как здесь всё устроено

Эту папку наполняет Chat Marker. Руками правь только `{FOLDERS['drafts']}` — остальное
пересобирается при каждой выгрузке, и правки там затрутся.

- `{FOLDERS['inbox']}` — сырые данные, единственный источник. Не удалять.
- `{FOLDERS['topics']}` — подборки по темам. Тема заводится в браузере при выделении.
- `{FOLDERS['chats']}` — то же, но разложено по страницам и чатам, откуда взято.
- `{FOLDERS['untagged']}` — то, что сохранено без темы. Разбери, когда дойдут руки.
- `{FOLDERS['docs']}` — живые документы по отмеченным темам, пересобираются сами.
- `{FOLDERS['drafts']}` — твоя территория.
- `{FOLDERS['archive']}` — прошлые версии выгрузки, последние {BACKUPS_KEEP}.
- `Индекс.md` — с чего начинать.
- `Выдержки.xlsx` — открой в Google Sheets, там фильтры по всем колонкам.
- `Дашборд.html` — открой двойным кликом, работает без интернета.
""")
...
def needs_rebuild(root: Path, paths: dict[str, Path]) -> bool:
    index = root / "Индекс.md"
    if not index.exists():
        return True
    # сборщик обновился — пересобираем, даже если выдержки те же.
    # Без этого новый дашборд не появлялся бы до следующей выгрузки.
...
class _Lock:
    """Блокировка между агентом launchd и MCP: одновременная сборка — гонка.

    Файл замка живёт в домашней папке, не на Google Drive — сетевые маунты
    flock переваривают плохо.
    """
    ...
    def __enter__(self):
        try:
            ...
            self.fh = None     # нет fcntl или прав — работаем без замка, как раньше
        return self
...
def log(msg: str) -> None:
    """Служебные сообщения — в stderr: stdout MCP-процесса занят протоколом."""
...
def _run_locked(do_ingest: bool = True, force: bool = False, quiet: bool = False) -> dict:
    """Один прогон: приём выгрузок плюс пересборка. Используется и из MCP-сервера."""
...
def main() -> int:
    ...
    if r["moved"]:
        print(f"  принято выгрузок: {r['moved']}")
    if r["skipped"]:
        print("  всё уже собрано, ничего не изменилось")
    elif not r["rebuilt"]:
        print("  выдержек пока нет, папка готова")
    else:
        print(f"  собрано: {r['items']} выдержек, {r['topics']} тем, {r['sources']} источников"
              + (f", без темы {r['untagged']}" if r["untagged"] else "")
              + (f", документов {len(r['docs'])}" if r["docs"] else "")
              + ("" if r["xlsx"] else " (без xlsx — нет openpyxl)"))
    print(f"  папка: {r['root']}")
    return 0
